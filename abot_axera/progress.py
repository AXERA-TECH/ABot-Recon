...
_S = {"phase": "", "frame": 0, "total": 0, "t0": 0.0, "t_phase": 0.0, "spf": None}
_last_spf: float | None = None


def clear() -> None:
    _S.update(phase="", frame=0, total=0, t0=0.0, t_phase=0.0, spf=None)
...
def start_infer(total: int, hint_spf: float | None = None) -> None:
    ...
    if not _S["t0"]:
        _S["t0"] = now
    _S.update(phase="infer", frame=0, total=int(total), t_phase=now, spf=_last_spf or hint_spf)
...
def upd(frame: int, total: int | None = None) -> None:
    ...
    if total is not None:
        _S["total"] = int(total)
    if _S["frame"] > 0:
        _S["spf"] = (time.time() - _S["t_phase"]) / _S["frame"]
        _last_spf = _S["spf"]
